...
phaselist = getFileList(dir + "Stack/"); //Vector of all the folders of phases
for (i=0; i<phaselist.length; i++) {
	phase = phaselist[i];
	File.makeDirectory(outputfol + phase); //Make output phase folder
	celllist = getFileList(dir + "Stack/" + phase);
	for (j=0; j<celllist.length; j++) {
		...
		print("Cell: " + p + "," + s);
		slist = getFileList(dir + "Stack/" + phase + cell);
		t = slist.length / 2 / 96; //Total images /2 channels /96 layers		
		cellimage = dir + "Stack/" + phase + cell + slist[1]; //first cell image
